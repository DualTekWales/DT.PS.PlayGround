...
#>
	...
	function Get-MyScheduledTask ($Computername)
	{
		$ScriptBlock = {
			...
			$Root.GetFolders(0) | ForEach-Object { $Folders.Add($_) | Out-Null }
	...
	try
	{
		...
		$SrcSchTasks = Get-MyScheduledTask | Where-Object { $ExcludePaths -notcontains $_.Path }
		...
		$UserAccountsAffected = $SrcSchTasks | ForEach-Object { ([xml]$_.xml).Task.Principals.Principal.UserId } | Select-Object -Unique
		if ($UserAccountsAffected)
		{
			$StoredCredentials = @{ }
			$UserAccountsAffected | ForEach-Object {
